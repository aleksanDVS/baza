...
def zapisz_dziennik(akcja, szczegoly):
    try:
        # Matches schema columns: data, akcja, szczegoly, uzytkownik
...
if menu == "📊 Dashboard":
    st.title("Statystyki")
    try:
        # Using exact casing from your schema
            ...
            with st.form("sale"):
                ...
                if st.form_submit_button("Sprzedaj"):
                    ...
                    if ile <= row['liczba']:
                        ...
                        supabase.table("Produkty").update({"liczba": nowa_liczba}).eq("id", pid).execute()
    ...
    try:
        with st.form("kat"):
            ...
            if st.form_submit_button("Dodaj"):
                ...
                zapisz_dziennik("KATEGORIA", f"Dodano {nowa}")
                st.rerun()
    ...
    st.title("Historia")
    try:
        # Added try-except specifically here to catch the APIError
        ...
        st.info("Sprawdź czy RLS jest wyłączone dla tabeli 'dziennik' w Supabase.")
